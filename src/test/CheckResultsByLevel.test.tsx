...
describe("CheckResultsByLevel", () => {
  ...
  it("renders the header check correctly when things make sense", () => {
    ...
    const totalChecksCounter = wrapper.find(
      "div.BackstageInfoCard-subheader-13",
    );
    expect(totalChecksCounter.length).toEqual(1);
    expect(totalChecksCounter.at(0).text()).toEqual(
      "Total Checks Passing: 2 / 4 (50%)",
    );
  });
  ...
  it("renders the right level headers", () => {
    ...
    expect(longLevelHeaderParagraphs.at(0).text()).toEqual(
      "very long level name (3)",
    );
    ...
    expect(anotherLevelHeaderParagraphs.at(0).text()).toEqual(
      "and another one (0)",
    );
