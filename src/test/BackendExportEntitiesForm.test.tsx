...
import BackendExportEntitiesForm, {
  sanitizeSchedule,
} from "../components/BackendExportEntitiesForm";
...
const renderComponent = async (
  config = CONFIG_ENABLED_EVERY_HOUR,
  runs: {
    total_count: number;
    rows: Array<AutoSyncExecution>;
  } = EXECUTION_RESPONSE,
) => {
  ...
  await waitFor(() =>
    expect(
      screen.getByTestId(runs.rows.length === 0 ? "no-run-msg" : "text-output"),
    ).toBeInTheDocument(),
  );
};
...
describe("BackendExportEntitiesForm", () => {
  it("displays the configuration if auto sync is on at every hour", async () => {
    ...
    expect(screen.getByTestId("autosync-cron")).toHaveTextContent(
      "EveryhourClear",
    );
  });
  ...
  it("displays the configuration if auto sync is off and set to every day at 5", async () => {
    await renderComponent({
      ...
      auto_sync_schedule: "0 5 * * *",
    });

    expect(screen.getByTestId("autosync-toggle")).not.toHaveClass(
      "Mui-checked",
    );
    expect(screen.getByTestId("autosync-cron")).toHaveTextContent(
      "Everydayat5 Clear",
  ...
  it("displays the execution header as expected if it is the first execution", async () => {
    ...
    expect(screen.getByTestId("execution-header-prev")).not.toHaveClass(
      "MuiLink-root",
    );
    expect(screen.getByTestId("execution-header-next")).toHaveClass(
      "MuiLink-root",
    );
  });
  ...
  it("displays the execution header as expected if it is the last execution", async () => {
    ...
    expect(screen.getByTestId("execution-header")).toHaveTextContent(
      "<<< Showing execution 1 of 1 >>>",
    );
    expect(screen.getByTestId("execution-header-prev")).not.toHaveClass(
      "MuiLink-root",
    );
    expect(screen.getByTestId("execution-header-next")).not.toHaveClass(
      "MuiLink-root",
    );
  });
  ...
  it("displays a message if there are no executions", async () => {
    ...
    expect(screen.getByTestId("no-run-msg")).toHaveTextContent(
      "Export has never run. Please ensure export is enabled in the configuration above.",
    );
  });
  ...
  it("displays the right header fields", async () => {
    ...
    expect(screen.getByTestId("run-started-at")).toHaveTextContent(
      "May 30th 2023, 19:45:24 (UTC)",
    );
    expect(screen.getByTestId("run-completed-at")).toHaveTextContent(
      "May 30th 2024, 19:45:24 (UTC)",
    );
  });
  ...
  it("displays the right text output", async () => {
    ...
    expect(screen.getByTestId("text-output")).toHaveTextContent(
      "very important",
    );
  });
