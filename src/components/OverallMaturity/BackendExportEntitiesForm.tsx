...
import { Button, Typography, Link, makeStyles } from "@material-ui/core";
...
export default function BackendExportEntitiesForm() {
  ...
  const [configuration, setConfiguration] =
    useState<AutoSyncConfiguration | null>(null);
  ...
  useEffect(() => {
    currentIndexRef.current = currentRunIndex;
  }, [currentRunIndex]);
  ...
  const setSchedule = (schedule: string) => {
    ...
    if (newSchedule !== configuration.auto_sync_schedule) {
      setConfiguration({
        auto_sync_enabled: configuration.auto_sync_enabled,
        auto_sync_schedule: newSchedule,
      });
      setDirty(true);
    }
  };
  ...
  const setEnabled = (enabled: boolean) => {
    ...
    setConfiguration({
      ...
      auto_sync_schedule: configuration.auto_sync_schedule,
    });
    setDirty(true);
  };

  const loadRun = (index = currentRunIndex) => {
    opslevelPluginApi.getAutoSyncExecution(index).then((ret) => {
      const contentChanged =
        ret.rows.length > 0 &&
        ret.rows[0].output !== currentRunRef?.current?.output;
  ...
  const saveConfiguration = () => {
    ...
    opslevelPluginApi
      .setAutoSyncConfiguration(configuration)
      .then((success) => {
        setDirty(!success);
  ...
  return (
    <>
      ...
      >
        ...
        >
          <Typography>
            <b>Configuration{dirty && " - unsaved changes"}</b>
          </Typography>
        </AccordionSummary>
        <AccordionDetails className={classes.accordionDetails}>
          {!!configuration && (
            <span>
              ...
              />
              <div
                className={classes.cronContainer}
                data-testid="autosync-cron"
              >
                <Cron
      ...
      {(runCount === null || runCount > 0) && [
        <div
          className={classes.executionNavHeader}
          key="nav-header"
          data-testid="execution-header"
        >
          {currentRunIndex > 0 && (
            <Link
              data-testid="execution-header-prev"
              href="#"
              onClick={prev}
          ...
          {currentRunIndex <= 0 && (
            <span data-testid="execution-header-prev">&lt;&lt;&lt;</span>
          )}
          &nbsp;
          <div className={classes.executionNavHeaderText}>
            Showing execution {currentRunIndex + 1} of {runCount}
          </div>
          &nbsp;
          {currentRunIndex < (runCount || 0) - 1 && (
            <Link
              data-testid="execution-header-next"
              href="#"
              onClick={next}
              className={classes.paginationLink}
            >
        ...
        <table key="info-table">
          <tbody>
            <tr>
              ...
              <td data-testid="run-state" className={classes.configCell}>
                {!!currentRun &&
                  currentRun.state.charAt(0).toUpperCase() +
                    currentRun.state.slice(1)}
              </td>
              <td className={classes.paddingCell} />
              <td className={classes.configHeaderCell}>Trigger</td>
              <td data-testid="run-trigger" className={classes.configCell}>
                {!!currentRun &&
                  currentRun.trigger.charAt(0).toUpperCase() +
                    currentRun.trigger.slice(1)}
              </td>
            </tr>
            <tr>
              <td className={classes.configHeaderCell}>Started at</td>
              <td data-testid="run-started-at" className={classes.configCell}>
                {currentRun
                  ? `${moment
                      .utc(currentRun.started_at)
                      .format("MMMM Do YYYY, HH:mm:ss")} (UTC)`
                  : ""}
              </td>
              <td className={classes.paddingCell} />
              <td className={classes.configHeaderCell}>Completed at</td>
              <td data-testid="run-completed-at" className={classes.configCell}>
                {currentRun?.completed_at
                  ? `${moment
                      .utc(currentRun.completed_at)
                      .format("MMMM Do YYYY, HH:mm:ss")} (UTC)`
                  : ""}
              </td>
            </tr>
        ...
        <Typography
          variant="body1"
          className={classes.outputComponentStyle}
          ref={outputRef}
          key="text-output"
        >
          {!!currentRun && (
            <span data-testid="text-output">{currentRun.output}</span>
          )}
        </Typography>,
      ...
      {runCount !== null && runCount === 0 && (
        <Typography data-testid="no-run-msg" style={{ margin: "15px" }}>
          Export has never run. Please ensure export is enabled in the
          configuration above.
        </Typography>
      )}
    </>
